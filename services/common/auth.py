...
def get_user_roles(user_groups):
    ...
    dqddma_user = any("admin" in name or "dqddma" in name for name in matched_lower)
        ...
        if isresearch_user == 'false' or isresearch_user == None:
            ...
            data_sources = {
                "ahrf": {
                    "label": "AHRF", 
                    "access": survey_user, 
                    "description": "Area Health Resources Files with U.S. health workforce and services data."
                },
                # "sohea": {"label": "SOHEA", "access": sohea_user, "description": "State of Oral Health Equity in America (SOHEA) survey."},
                "hpsa": {
                    "label": "HPSA", 
                    "access": survey_user, 
                    "description": "Medical and Dental U.S. Health Provider Shortage Areas."
                },
                "merative": {
                    "label": "Merative", 
                    "access": merative_user, 
                    "description": "Merative dental and medical claims data."
                },
                "dqddma": {
                    "label": "DQ-DDMA", 
                    "access": dqddma_user, 
            ...
            for key, val in data_sources.items():
                user_info["datasourcesAccess"].append({
                    ...
                    "questions": source_specific_user_prompts_guide_book.get(key)
                })
